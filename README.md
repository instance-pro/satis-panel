...
| `REDIS_URL` | set in compose | Redis for the webhook request log and the build queue, `redis://:<password>@redis:6379`. Empty disables both. |
...
With Redis configured (default in `docker-compose.yml`) builds from webhooks, the
